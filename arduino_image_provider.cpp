...
/*
 * On the OV7670 the resolutions QVGA and QQVGA are downsampled
 * without cropping, thus keeping the full field of view.
 * If we use QQVGA we have the full field of view downsampled to the
 * resolution (160 x 120) closest to the one of out final frame (96 x 96)
 * thus minimizing the FOV lost during cropping.
 * On the OV7675 the sub resolutions are probably all cropped so this
 * would not work.
 */
#define RESOLUTION QQVGA  // VGA,     QVGA,     QQVGA,     QCIF
#define RESX 160          // VGA 640, QVGA 320, QQVGA 160, QCIF 176 
#define RESY 120          // VGA 480, QVGA 240, QQVGA 120, QCIF 144

// Get an image from the camera module
TfLiteStatus GetImage(tflite::ErrorReporter* error_reporter, int image_width,
                      int image_height, int channels, int8_t* image_data) {

  byte data[RESX * RESY]; // Receiving grayscale from camera = RESX * RESY * 1
  ...
  if (!g_is_camera_initialized) {
    if (!Camera.begin(RESOLUTION, GRAYSCALE, 5, OV7670)) {
  ...
  for (int y = min_y; y < min_y + 96; y++) {
    for (int x = min_x; x < min_x + 96; x++) {
      image_data[index++] = static_cast<int8_t>(data[(y * RESX) + x] - 128); // convert TF input image to signed 8-bit
    }
  }
